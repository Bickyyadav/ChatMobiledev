...
export const initializeSocket = (httpServer: HttpServer) => {
    ...
    // it's the event that is triggered when a new client connects to the server that new client came online bicky came online and everyone will trigger that bicky came onlin to my message 

    io.on("connection", (socket) => {
        // we have to store all the connected user id's in a map that is io.use  line no 27 socket.data.userId = user._id.toString();
        ...
        // handle sending messages listening joe send to server and server to bicky 
